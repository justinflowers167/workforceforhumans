...
- **Deploy an Edge Function**: `supabase functions deploy <name>` (requires Supabase CLI and project link to `dbomfjqijyrkidptrrfi`). The nine functions are `create-checkout`, `stripe-webhook`, `link-employer`, `parse-resume`, `match-jobs`, `send-match-digest`, `intelligence-feed`, `refresh-jobs`, `prune-inactive-data`.
