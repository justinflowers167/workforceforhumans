...
- **`parse-resume`** and **`match-jobs`** use the Anthropic SDK with model **`claude-fable-5`** (upgraded from Sonnet 4.6 on 2026-06-10 — both outputs are member-facing coach prose, the product's moat surface; ~3× per-run cost, revert lever documented in runbook §6). Both parse model output by slicing from the first `{` to the last `}` — the system prompts instruct JSON-only output but the slice is defensive. Keep this pattern if you add another Claude call.
- **`parse-resume`** downloads from the `resumes` storage bucket. **PDFs are sent directly to Claude as a `document` content block** — `pdf-parse` was dropped in Phase 9 hotfixes (2026-04-25) because esm.sh kept failing to serve it on Deno cold-starts; Claude reads PDFs natively with better layout understanding (columns, tables) than text-stripping anyway. **DOCX still uses `mammoth` (lazy-loaded inside the function)** so a future esm.sh hiccup with mammoth returns a clean 500 instead of crashing the whole function. After parsing, `parse-resume` updates `resumes`, `job_seekers`, and syncs `skills` + `job_seeker_skills` — upserting by lowercased name. The `is_current=true` re-assert at the end of a successful parse is now redundant with the `trg_resumes_flip_others` DB trigger (which enforces the one-current invariant in the same transaction as any insert/update) but harmless.
- **`match-jobs`** scores up to 50 active jobs for the authenticated seeker. The system prompt asks Claude to filter to score ≥ 40 and sort desc; the code then `.slice(0, 10)` on whatever the model returns (no code-side score filter). Before insert it deletes the seeker's non-emailed `match_scores` rows for `match_type = 'job'`, then inserts the fresh top-N. Phase 13 (2026-04-28) added a per-match coach brief alongside the existing rationale/growth_note: three new prose fields (`resume_tailoring`, `skill_gap_plan`, `application_strategy`) generated in the same Claude call. The seeker payload now includes the current resume's `raw_text` (capped 4000 chars) + `parsed_json` so the brief references real bullets instead of generic advice. `max_tokens` 6000 → 8000 to absorb the extra output. Prompt-cache marker on the system block is a silent no-op on `claude-fable-5` (2048-token minimum cacheable prefix; the system prompt is ~1.2k tokens) — harmless, and live again if the prompt grows past the floor.
...
- **Matches**: `match_scores` is polymorphic via `match_type` (`'job'` is the only type currently used). `similarity` is 0–1, `score` is 0–100 — both are written. `emailed_at` gates digest sending. Phase 7 added `rationale` + `growth_note` (member-facing prose surfaced under `<details class="match-why">` in `member.html`). Phase 13 (2026-04-28) added three more nullable text columns for the per-match coach brief: `resume_tailoring`, `skill_gap_plan`, `application_strategy` — populated by `match-jobs` in the same Claude call, surfaced as separate sub-sections under the same disclosure. All five prose fields are nullable so pre-Phase-7/Phase-13 rows render cleanly (each section hides if its field is null). When `skill_gap_plan` is present, the standalone "Next edge:" line collapses (`growth_note` and `skill_gap_plan` cover the same ground at different granularity; the prompt explicitly ties them together).
...
- **Model ID**: use `claude-fable-5` only where the output IS the product — member-facing coach prose (`match-jobs` briefs, `parse-resume` reviews; upgraded 2026-06-10 at ~3× Sonnet cost, revert lever in runbook §6). Use `claude-sonnet-4-6` for new structured/quality calls that aren't moat surfaces, and `claude-haiku-4-5` for binary or tightly-scoped classification — `refresh-jobs` switched to Haiku in Phase 10 §A for ~5× cost savings on its keep/reject filter; don't put Fable on crons. Fable 5 API notes: no `temperature`/`top_p`/`top_k`, no `budget_tokens`, no assistant-prefill, and never send `thinking: {type: "disabled"}` (omit the param instead) — all four return 400. Keep `max_tokens` bounded (current functions use 200–8000).
