...
**Follow-on PR — seeker-side coaching paused (claude/free-for-everyone-coaching-paused, shipped 2026-05-25):**

Founder decision same-day extending the launch-quarter free framing to the seeker side. The previous 3-plan coaching grid on `learn.html` (`$0` Self-Guided / `$79` Career Boost / `$249/mo` 1:1 Coaching) didn't have any paying customers either, and committing to paid 1:1 coaching while balancing a day job was setting up under-delivery against a paid promise. Decision: pause the paid plans entirely, lean into a "free for everyone during launch quarter" brand stance, add a soft donation ask for true believers.

- **`learn.html` Coaching Plans block** replaced. Old: 3-tier `.plans-grid` with `$0/$79/$249` cards and `.plan-card` styling. New: single `.free-card` titled "Your full Workforce for Humans toolkit" listing the 6 things every visitor gets free (Adaptation Assessment, named pathways, per-match coach brief, curated learning paths, weekly market briefing, Friday match digest), with one CTA to `/resume.html`. Practitioner-voice intro line names the constraint honestly: *"I'm a solo founder building this around a day job. Paid coaching is paused while I can't deliver it the way it deserves to be delivered."*
- **Soft donation ask** added beneath the free toolkit as a separate `.support-card`. Copy is explicit about why: *"The AI costs — resume parsing, match scoring, weekly intelligence — come out of my pocket while we build market credibility. If you've got the means and the platform's helped you, dropping a note is the entire ask. A donation page is on the way; for now, an email starts the conversation."* Link is `mailto:hello@workforceforhumans.com?subject=I%20want%20to%20support%20WFH` — no Stripe / GitHub Sponsors / Buy Me a Coffee integration yet (avoids half-built donation infrastructure; founder swaps in a real payment link when there's actual donation interest).
- **Dead CSS removed.** `.plans-grid`, `.plan-card`, `.plan-badge`, `.plan-name`, `.plan-price`, `.plan-curr`, `.plan-per`, `.plan-feats`, `.plan-btn` rules deleted (only the now-removed pricing grid used them). New `.free-card`, `.free-badge`, `.free-h3`, `.free-feats`, `.free-cta-row`, `.support-card`, `.support-eyebrow`, `.support-h3`, `.support-p`, `.support-link` styles added. Responsive `.plans-grid` rules at 1200px / 960px breakpoints also removed; the new layout is single-column at all sizes (max-width 780px centered).
- The `id="energy"` anchor on the `<section>` stays (a learn.html path-tab links to it for backward compat) even though the section content is no longer about Green Energy.

**Cost-economics implication** (per `feedback_ai_cost_economics.md`): with both seeker and employer sides free, all Claude API + OpenAI embedding + Resend email spend is founder-funded during the launch quarter. Current rate is bounded (~$10–50/mo at low traffic per the live Supabase data showing only the founder's test account); could scale to $200–2000/mo if seeker acquisition picks up post-Cluster-B. Donation income (if any) partially offsets. The cost-economics gate remains intact: if seeker traffic surges and API spend outpaces donation income meaningfully before the Adaptation Index Q3 ships, we revisit either re-introducing a paid tier or rate-limiting AI-heavy features (e.g., per-account match-job-regeneration cap).
